 ...
 * @LastEditTime: 2019-09-03 19:26:34 -0700
 * @LastEditors: 
 * @Description: 
 */
#include "head.h"
...
GtkWidget *create_button1(void)
{
    ...
    gtk_widget_set_size_request(GTK_BOX(box), 160, 160);
...
GtkWidget *ButtonLogo(void)
{
    ...
    image = gtk_image_new_from_file("./pic/minilogo.png");
    box = gtk_hbox_new(FALSE, 0);
    gtk_container_set_border_width(GTK_CONTAINER(box), 20);
    gtk_box_pack_start(GTK_BOX(box), image, TRUE, TRUE, 0);
    gtk_widget_set_size_request(GTK_BOX(box), 405, 160);
    gtk_widget_show(image);
    align = gtk_alignment_new(0, 0, 0, 0);
    gtk_container_add(GTK_CONTAINER(align), box);
    gtk_widget_show(box);
...
GtkWidget *CreateLoading(void)
{
    ...
    gtk_window_set_position(GTK_WINDOW(window), GTK_WIN_POS_CENTER);
    gtk_container_set_border_width(GTK_CONTAINER(window), 0);
    gtk_widget_set_size_request(GTK_WINDOW(window),405, 370);
    gtk_window_set_resizable(GTK_WINDOW(window), FALSE);
    //显示logo名
    titlebox = gtk_vbox_new(FALSE, 0);
    gtk_container_add(GTK_CONTAINER(window), titlebox);
    logobutton = ButtonLogo();
    gtk_box_pack_start(GTK_BOX(titlebox), logobutton, FALSE, FALSE, 5);
    sep = gtk_vseparator_new();
    gtk_box_pack_start(GTK_BOX(titlebox), sep, FALSE, FALSE, 5);

    hbox = gtk_hbox_new(FALSE, 0);
    gtk_box_pack_start(GTK_BOX(titlebox), hbox, FALSE, FALSE, 5);
    //显示头像
    ...
    gtk_box_pack_start(GTK_BOX(box1), entry1, FALSE, FALSE, 3);
    gtk_entry_set_max_length(entry1, 10);
    //一键删除
    deletebutton = ButtonDelete();
    gtk_box_pack_start(GTK_BOX(box1), deletebutton, FALSE, FALSE, 3);
    ...
    gtk_box_pack_start(GTK_BOX(box2), entry2, FALSE, FALSE, 3);
    gtk_entry_set_max_length(entry2, 20);
    //显示密码
    eyebutton = Buttoneye();
    gtk_box_pack_start(GTK_BOX(box2), eyebutton, FALSE, FALSE, 3);
