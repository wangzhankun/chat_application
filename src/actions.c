 ...
 * @LastEditTime: 2019-09-05 04:52:49 +0800
 * @LastEditors: 
 * @Description: 
 */
#include "head.h"
...
void* receiveFileFromServer(void* skf)
{
    pthread_detach(pthread_self());
    socketfd skfd = *(socketfd*)skf;
    Rec_File_From_Client(skfd);
}
...
void *createNewSKFToReceiveMSG(void *skf)
{
    ...
    {
        pthread_t tid;
        pthread_create(&tid,NULL, receiveFileFromServer, sock);
    }
    else
    {
        PushMessage(buf);
